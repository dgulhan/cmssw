...
HiFJGridEmptyAreaCalculator::HiFJGridEmptyAreaCalculator(const edm::ParameterSet& iConfig):
  _gridWidth(iConfig.getUntrackedParameter<double>("gridWidth",0.005)),
  _band(iConfig.getUntrackedParameter<double>("bandWidth",0.2))
{
  ...
  produces<std::vector<double > >("corrWKtArea");
...
{
  using namespace edm;
  //Get the vector of PF cands for empty space estimation
  // edm::Handle<edm::View<reco::PFCandidateCollection> > pfCands;
  ...
  std::auto_ptr<std::vector<double>> corrWKtAreaOut ( new std::vector<double>(neta-1,0));
  ...
  for(int ieta = 0; ieta<(neta-1); ieta++) {
   ...
   _eta_min_jet = eta_min + _band;  
   _eta_max_jet = eta_max - _band;  
  ...
  iEvent.put(corrWKtAreaOut,"corrWKtArea");
...
HiFJGridEmptyAreaCalculator::setup_grid_jet(const reco::Jet *jet) {
  ...
  assert(_ymaxjet - _yminjet >= _gridWidth);

  // this grid-definition code is becoming repetitive -- it should
  // probably be moved somewhere central...
  double ny_double = (_ymaxjet-_yminjet) / _gridWidth;
